...
about flowers, cars, life expectancy, or Big Five personality scores.
...
```{r}
(clevel <- haven::read_spss(here("data", "s008_data", "clevel.sav")))
```
...
ggsave(here::here("output", "clevel_extraversion.svg"), clevel_plot)
```
...
printed). Always use at least **300 DPI** resolution. The two most used bitmap
formats are `.tiff`/`.tif` and `.png`. `.tiff` is higher quality, but the files
can end up being fairly large. `.png` is smaller, but it supports fewer colors.
Stick with `.tiff` unless you intend to host these images on a web page; then,
`.png` will help your pages load faster generally.

Don't use `.jpg`/`.jpeg` (it's lossy) and don't use `.gif` unless the plot is 
animated.

Let's save to some other formats:

```{r}
ggsave(here::here("output", "clevel_extraversion.eps"), clevel_plot)
ggsave(here::here("output", "clevel_extraversion.pdf"), clevel_plot)
ggsave(here::here("output", "clevel_extraversion.tiff"), clevel_plot)
ggsave(here::here("output", "clevel_extraversion.png"), clevel_plot)
```
