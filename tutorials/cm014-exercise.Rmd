...
When variables are not independent, then we can gain information about one 
variable if we know something about the other.

**Example:** Use the scatterplot below
...
If exploratory data analysis isn't enough, we can answer these questions more
precisely by _fitting a model_: a curve that predicts _Y_ given _X_. This is 
also called a __regression curve__ or a __machine learning model__. 
...
In R, takes the form `y ~ x1 + x2 + ... + xp` (x1, x2, etc. are the column names 
in your data frame).
...
```{r}
(mod_france <- lm(FILL_THIS_IN))
```

Does that mean that the life expectency at "year 0" was equal to -397.7646?!

We are interested in the modeling results around the modeling period which 
starts at year 1952. To get a meaniningful "interpretable" intercept, we can use 
the `I()` function. `I()` lets you tell R to fit the model to an arithmetic
transformation of a variable. For example:

```{r}
(mod_france <- lm(lifeExp ~ I(year - 1952), data = gapminder_France))
