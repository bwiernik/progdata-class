...
-   [haven](https://haven.tidyverse.org/) package for importing SPSS,
    SAS, Stata files
-   [rio::import()](https://cloud.r-project.org/web/packages/rio/vignettes/rio.html)
    for importing all manner of formats with one command!
...
```{r, eval = FALSE}
dat <- read_csv(here::here("data", "s008_data", "gap_asia_2007.csv"))
dat
```
...
```{r, eval = FALSE}
dat <- read_csv(
  here::here("data", "s008_data", "gap_asia_2007.csv"),
  col_types = cols(
    country = col_factor(),
...
```{r, eval = FALSE}
(clevel <- haven::read_spss(here::here("participation", "data", "clevel.sav")))
```
...
```{r, eval = FALSE}
ggsave(here::here("participation", "output", "figures", "clevel_extraversion.pdf"), clevel_plot)
